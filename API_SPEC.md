...
服务端代理 Aliyun Bailian Qwen（`qwen-mt-flash`/`qwen3.5-ocr`/`qwen3-tts-instruct-flash`/`qwen3-asr-flash`），Key 只在服务端读取。客户端（含 Android/iOS）禁止直连 DashScope/Bailian。当前 Android/iOS 尚未接入 Translator，接入时按此路由字段对齐，不新起翻译服务。
