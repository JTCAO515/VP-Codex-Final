...
ADR-045: Native mobile exploration via React Native + Expo (historical, superseded for Android by ADR-091).
...
- Decision at the time: recommend React Native + Expo. This was later superseded for the active Android APK line by `v0.3.2` ADR-091, which uses Kotlin + Jetpack Compose + Room/DataStore/MVI as the Android baseline while still reusing backend contracts.
...
## v0.2.17 Design Update - Data Provider Expansion Assessment (docs only)

No new ADR (a pure evaluation of external services against the existing Amap/ExploreProvider abstraction and the standing "apply officially, never scrape/reverse engineer" principle already established for Dianping/Meituan and FlyAI — no new architectural decision was required). Key judgment recorded here for future agents:

- The evaluation criterion is deliberately *not* "how complete is the data" but "how close does this get a foreign FIT traveler to actually paying for a ticket/room inside the app" — Amap already solves the map-POI layer; the real gap is bookability, which none of the existing providers touch.
- Trip.com (hotels) and Klook (tickets/activities) are recommended as the top two research priorities because both are foreigner-friendly by design (English UI, international card payment) and would slot into the existing provider-abstraction pattern (`lib/explore/amapProvider.ts`-style: server-side route proxy, server-only keys, AI-generated content retained as fallback) without introducing a new architectural shape.
...
- Reason: 原生技术栈能提供最佳的图片解码与高德地图硬件加速渲染，保障 60fps 滚动流畅度；原生 SDK 对系统权限（相机、定位）和生命周期具有更可靠的掌控，能最大程度降低内存溢出和后台服务被杀的概率。
...
## v0.3.2 Design Update - Android Planning Synthesis

ADR-091: Android native engineering baseline is retained, but the product navigation is Today-first.

- Background: The parallel GitHub planning document gave strong native engineering detail, while Codex's local planning emphasized the traveler's real on-the-ground workflow. These needed to be merged before implementation.
- Decision: Keep Kotlin + Jetpack Compose + Material 3 + Room + DataStore + MVI/StateFlow as the Android implementation baseline, but organize the product around Today / Butler / Plan / Explore / Tools. The old Canvas concept remains as the itinerary content model inside Today and Plan.
- Reason: Engineering rigor without traveler-centered navigation risks building a technically polished but less intuitive app. Today-first navigation makes the APK prove immediate value.

ADR-092: Taxi Driver Card access must be visible and intentional.

- Background: The v0.3.1 planning draft included hidden/global launch ideas for the taxi card. The operator explicitly asked to remove those.
- Decision: Taxi Driver Card entry points are visible UI actions from Today, the current trip/day card, and Day Detail. Do not use hidden or background resident shortcut behavior as roadmap requirements.
- Reason: Visible actions are easier to discover, safer for privacy/battery, simpler to test, and less likely to conflict with Android system policy.

ADR-093: v0.3.3 starts with Android Native Foundation, not full map/translator/booking scope.

- Background: The parallel plan included map, translator, Room, MVI, Tools, and taxi-card details in the early native direction. That is useful but too broad for the first implementation.
- Decision: v0.3.3 should build the native app foundation and five-surface shell with local mock trip data. Today + Plan execution follows in v0.3.4, Butler + sync in v0.3.5, Translator in v0.3.6, and Explore candidates in v0.3.7.
- Reason: The project needs a shippable APK base before complex provider/device integrations.
...
## v0.3.7 Design Update - Figma Make Visual Reference Alignment (Visual-Only)

ADR-105: Android's color hex values now intentionally diverge from the web app's `app/globals.css`, as a scoped, operator-approved exception to ADR-094/095's "verbatim mirror" rule.

- Background: the operator provided a Figma Make file, "Design According to MD Document" (https://www.figma.com/make/8J1WnuwCHwx60bSC6f7fQn/), and asked to adopt its layout. Reading the file (via the Make project's own code editor view, since the Figma MCP's `get_design_context` only returns a resource-link manifest for Make files rather than inlined code) revealed a materially different product structure — five screens named Chat(home)/Plan/Explore/Tools/**Me** with a two-side-plus-center-FAB bottom nav, versus the repo's actual Today/Chat/Plan/Explore/Tools horizontal nav. Asked the operator via `AskUserQuestion` how much to adopt; the operator chose "visual layer only."
- Decision: `Color.kt`'s `Paper`/`Cinnabar`/`Gold`/etc. now use the Figma reference's exact hex values (`#FAF8F4`, `#C1292E`, `#C9A84C`, and related ink/muted tones) rather than the values ported from `app/globals.css` in ADR-094/095. This round did not touch `app/globals.css` itself, so the two platforms now carry deliberately different — though visually related, same warm-paper/cinnabar-red/gold family — hex values.
- Reason: the operator explicitly approved adopting the Figma palette for Android specifically, without asking for a matching web change this round. This is recorded as a conscious, scoped divergence rather than silently letting ADR-094/095 quietly go stale — a future round should either reconcile the two (port the new Android values back into `globals.css`, if the operator wants unified branding) or explicitly re-affirm the divergence, not assume nobody wrote it down.

ADR-106: Android bundles its first custom fonts — Playfair Display and DM Sans — ending the v0.3.3-era "system font placeholder" decision.

- Background: `Type.kt`'s original doc comment (v0.3.3) explicitly deferred bundling a custom serif, citing APK size and licensing review as out of scope for the foundation round. The Figma reference specifies Playfair Display for headline/brand moments and DM Sans for all other UI text.
- Decision: downloaded both as variable-font `.ttf` files from Google's `google/fonts` OFL-licensed repository (no static per-weight files exist upstream for either family, only single variable-axis files), placed them in `res/font/`, and referenced specific weights via `Font(resId, weight, variationSettings = FontVariation.Settings(FontVariation.weight(...)))` — one file per family covers every weight needed, keeping APK size lower than bundling a static file per weight. `FontVariation` is an experimental Compose Text API and required `@OptIn(ExperimentalTextApi::class)` to compile — the same class of gap as the `TopAppBar`/`ExperimentalMaterial3Api` issue found in v0.3.5's build-verification pass.
- Reason: OFL (SIL Open Font License) fonts are free to bundle in an app with no licensing review needed, closing the original v0.3.3 concern; the licensing question that justified deferring this is now resolved, so there was no reason to keep waiting.

ADR-107: The Figma Make reference's product-structure ideas (a "Me" tab replacing Today, a two-side/center-FAB bottom nav, an early Needs Scheduling section in Plan, an 8-utility Tools grid, a full-screen Translator overlay) were deliberately not adopted this round — this extends ADR-099's "Figma Make is a design reference, not a behavioral source of truth" beyond passive advice into an actual decision point that got tested.

- Background: this is the first round where a Figma Make reference's product structure genuinely conflicted with the repo's actual roadmap (rather than just being visually different), so ADR-099's principle needed to be actively applied, not just cited.
- Decision: kept the existing five-surface Today/Chat/Plan/Explore/Tools horizontal `NavigationBar`, kept Today as a real surface (not replaced by "Me"), left Plan's Needs Scheduling work deferred to v0.3.9 as already planned, and did not build a Tools grid or Translator overlay this round.
- Reason: the operator was asked directly and chose the narrower scope; adopting the wider structural change would have been a roadmap-altering decision that ADR-099 already says shouldn't be made unilaterally from a design reference alone.

ADR-108: The Taxi Driver Card's large-Chinese-character text now uses 52sp (the Figma spec) with a matching 64sp line height — fixing a real overlapping-glyph bug that the original 34sp size had been too small to expose.

- Background: `TaxiDriverCardDialog`'s address `Text` used `MaterialTheme.typography.headlineMedium.copy(fontSize = 34.sp)`, inheriting `headlineMedium`'s 32sp line height without noticing that 32sp line height under 34sp font size was already tight. Bumping only `fontSize` to the Figma-specified 52sp (without touching line height) made multi-line Chinese addresses render with visibly overlapping character strokes — confirmed via an actual emulator screenshot before the fix, not caught by the compiler since this is a runtime layout issue, not a type error.
- Decision: `copy(fontSize = 52.sp, lineHeight = 64.sp)` — both values changed together. Re-verified via emulator screenshot that a real three-line address ("北京市东城区景山前街4号") now renders with clean line separation and no glyph overlap.
- Reason: font size and line height are coupled — future changes to either value on this Text (or any other multi-line large-text display in the app) must consider both together, not adjust one and assume the other was already fine.

ADR-109: Camera/Mic composer buttons are visual-only placeholders, but Taxi Card's Speak button is wired to real `TextToSpeech` — the deciding factor is whether the feature needs a runtime permission, not whether it's "just UI."

- Background: the operator followed up by supplying the Figma Make project's actual local source export (not just the browser-scraped excerpt used for the first visual pass), which surfaced two UI details missed initially: the Chat composer's Camera/Mic icon buttons, and a Speak button next to Copy on the Taxi Driver Card. Asked the operator whether to add these; the operator said yes.
- Decision: `ButlerScreen.kt`'s composer now renders Camera/Mic `IconButton`s in a disabled state — they render for visual parity with the Figma reference, but do nothing on click and stay disabled, because wiring real camera capture or speech-to-text would require requesting the camera/microphone runtime permissions ahead of the point-of-use policy the project already holds to for the Translator round (v0.3.8). `TaxiDriverCard.kt`'s new Speak button, by contrast, calls Android's built-in `TextToSpeech` API directly and for real — `TextToSpeech` requires no runtime permission grant, so wiring it doesn't cross the same line the Camera/Mic buttons would.
- Reason: "visual layer only" (per the v0.3.7 operator decision, ADR-107) isn't the same test as "does this need a permission." A feature that's genuinely free to wire for real (no permission, no new capability surface) should be wired for real rather than faked, even inside a nominally visual-only round; a feature that would require expanding the permission surface stays a placeholder until its dedicated round. Verified on an Android 34 emulator: `TextToSpeech` initialization triggers a real on-device download of the zh-CN voice pack (logged via `GoogleTTSServiceImpl`), the Speak button flips from disabled to enabled once that completes, and tapping it dispatches a real Chinese speech-synthesis request with no crash.

## v0.3.8 Design Update - Bottom-Nav Product Restructure: Trips / Explore / Chat / Tools / Me

ADR-110: The bottom nav is restructured to two side pairs flanking a raised center Chat button, and Chat becomes visually dominant — a deliberate reversal of v0.3.7's "keep the nav shape unchanged" decision, made by explicit fresh operator instruction, not a unilateral re-adoption of the Figma reference's structure.

- Background: v0.3.7 (ADR-107) explicitly declined to adopt the Figma Make reference's two-side/center-FAB bottom nav shape, scoping that round to visual-only changes. The operator then separately, explicitly asked: "底部导航栏顺序Trips/Explore/Chat/Tools/Me（account），Chat要突出" (bottom nav order Trips/Explore/Chat/Tools/Me, Chat needs to stand out). This is a new, distinct instruction — not the operator reversing course on the earlier "visual only" answer, but a later request to actually make the structural change that was previously declined.
- Decision: `TopLevelDestination.leftOfCenter = [Trips, Explore]`, `rightOfCenter = [Tools, Me]`, with `Butler` (Chat) rendered as a separate, raised 58dp filled circle (`Modifier.shadow` + `CircleShape`, offset -14dp above the bar) rather than a fifth `NavigationBarItem`. `VisePandaBottomBar.kt` was rewritten from a plain Material 3 `NavigationBar` loop to a custom `Box`-over-`Surface` layout to achieve this.
- Reason: this documents the precedent correctly — ADR-107's "we didn't adopt X" is not a permanent ban on X; it was scoped to that round's operator-approved decision. A future agent reading ADR-107 alone, without this entry, might wrongly conclude the center-FAB nav shape was rejected outright rather than deferred pending a direct ask.

ADR-111: Trips merges Today and Plan into one destination; Me is new and only its active-trip data is real.

- Background: the operator's restructure request removed Today as an independent bottom-nav entry point (in favor of Trips/Explore/Chat/Tools/Me — no Today, no Plan as separate items). Asked the operator via `AskUserQuestion` what "Trips" should mean, given Today already had real, shipped content (readiness score, Now/Next/Later timeline, Ask Butler entry) that couldn't just be silently dropped; the operator chose "Trips = merge Today + Plan."
- Decision: `ui/trips/TripsScreen.kt` renders Today's readiness/timeline/Ask-Butler content at the top, followed by Plan's day-by-day list below, backed by one `TripsViewModel` that combines what were previously two separate ViewModels' data sources. `ui/today/` is deleted outright (not deprecated-in-place) since nothing else referenced it. `ui/me/MeScreen.kt` is new: only the "active trip" row is real (sourced from the same `TripRepository` every other screen reads), and the Preferences/Data & Privacy sections are explicitly labeled as local example data pending a real account system — following the same "honest placeholder, not a fake feature" discipline the project already applies to Explore/Tools placeholders.
- Reason: merging screens that already have real, tested content is a higher-risk refactor than building a new placeholder — DayDetail's route and `TripRepository`'s interface were deliberately left untouched so this merge is additive at the navigation layer, not a data-layer rewrite.

ADR-112: Every Material 3 `ColorScheme` role must be explicitly assigned, not left to `lightColorScheme()`'s baseline defaults — discovered via a real, pixel-sampled rendering bug where `Card`-embedded `Text` with no explicit color rendered in an unintended reddish tone.

- Background: while building and screenshot-verifying the new Trips/Me screens, `Text` composables inside `Card`s that had no explicit `color` parameter (relying on the ambient content color Material 3 provides) rendered in a color close to `error`/`CinnabarDeep` instead of a neutral `onSurface`/`onSurfaceVariant` tone. This was confirmed empirically, not by eye: cropping the actual device screenshot and sampling pixel RGB values with Python/Pillow showed the rendered color (~`(160-164, 35-42, 39-46)`) matched `CinnabarDeep` (`0xFFA02226`) almost exactly, while `Text`s with an explicit `onSurfaceVariant`/`onSurface` color at the same visual hierarchy level correctly sampled to the intended dark ink tone. This bug pre-dates this round — it was present in `ButlerScreen.kt`'s `EmptyButlerPrompt`/`MessageBubble` and `DayDetailScreen.kt`'s `BlockDetailCard`/`BookingCandidateRow` since whichever earlier round wrote them, just never caught because no prior verification pass pixel-sampled screenshot colors rather than eyeballing them.
- Decision (two attempts): First, fully specified all ~30 Material 3 `ColorScheme` roles in `Theme.kt` (previously only ~16 were set, leaving `surfaceContainer*`, `surfaceTint`, `errorContainer`, `outlineVariant`, `scrim`, etc. to Material's off-brand baseline purple defaults) — re-tested with the same pixel-sampling method and confirmed this did **not** fix the bug. Second (the actual fix): added explicit `color = MaterialTheme.colorScheme.onSurface` (or `onSurfaceVariant` where appropriate) to every affected `Text` across `ButlerScreen.kt`, `DayDetailScreen.kt`, `TripsScreen.kt`, and `MeScreen.kt`, verified via the same pixel-sampling method to confirm the fix. The fully-specified `ColorScheme` from the first attempt was kept anyway (still better practice, just not the fix for this particular symptom).
- Reason: this project's non-baseline, heavily-customized color scheme makes Material 3's default `Card`/`Surface` content-color resolution unreliable in ways that are easy to miss by eye (the wrong color is a plausible-looking dark-ish red, not an obviously broken color like magenta), and screenshots viewed casually can look "close enough" to correct even when they aren't. **Future color-correctness verification for this app must sample actual pixel values from real screenshots (e.g. via Python/Pillow), not rely on visual inspection alone** — this is now the standard the project holds itself to for any future color-related bug report or fix claim.

ADR-113: The bottom nav becomes a floating, fully-rounded pill inset from all three screen edges, replacing v0.3.8's flush-to-edge full-width bar; `OutlinedTextField`/`AlertDialog` corners are enlarged; the Taxi Driver Card stays (not removed) but gets its dialog-button overflow bug fixed as a side effect.

- Background: the operator supplied a five-screen (Chat/Plan/Explore/Tools/Me) black-mockup design reference and three direct instructions: (1) "导航栏要做圆角悬浮式" (the nav bar should be a rounded floating style), (2) "所有的文本框，对话框都做成圆角的，弧形" (all text fields and dialogs should be rounded/arc-shaped), (3) "taxi driver card 不重要 优先级极低，如果影响了项目进展可以移除" (the Taxi Driver Card is low priority — remove it if it's blocking progress).
- Decision (nav): `VisePandaBottomBar.kt`'s outer `Surface` changed from `fillMaxWidth().height(BottomNavHeight)` with `colorScheme.surface` (paper-white, flush to all edges) to a `RoundedCornerShape(Dimens.RadiusPill)`-shaped `Surface` filled with `Ink` (`0xFF1C1410`), wrapped in a `Box` padded `BottomNavHorizontalInset`/`BottomNavBottomInset` (both 16dp) away from the screen edges, at a shorter `BottomNavFloatingHeight` (64dp vs the old 80dp). Side nav items (`Trips`/`Explore`/`Tools`/`Me`) dropped their text labels — icon-only, matching the reference — with unselected icons tinted `Paper.copy(alpha = 0.55f)` for contrast against the dark pill (previously `onSurfaceVariant`, which assumed a light bar background) and selected icons keeping `colorScheme.primary` (Cinnabar). The center Chat `CircleShape` button is unchanged in shape/color, just re-anchored to the new shorter bar.
- Decision (text fields/dialogs): `ButlerScreen.kt`'s composer `OutlinedTextField` gained `shape = RoundedCornerShape(Dimens.RadiusXL)` (20dp — already an existing token, no new one added). `TaxiDriverCard.kt`'s `AlertDialog` gained the same `shape = RoundedCornerShape(Dimens.RadiusXL)`. Verified via emulator screenshot that both are now visibly rounded, not just default Material shapes.
- Decision (Taxi Driver Card): kept, not removed — none of this round's changes actually blocked on it; the operator's instruction was conditional ("if it affects project progress"), not a standing removal order, and `AGENTS.md`'s existing rule that the card must stay reachable only via a visible button (not a hidden trigger) still applies. While rounding the `AlertDialog`, manual verification surfaced a real, pre-existing (not introduced by this round) layout bug: the `confirmButton` Row's three `TextButton`s (Copy/Speak/Close) didn't fit the dialog's width, so `Close` got squeezed into an unreadably narrow column. Fixed by changing that `Row` to `FlowRow` (`Arrangement.End`, wraps to a second line when needed) — confirmed via a fresh emulator screenshot that `Close` now renders as readable wrapped text instead of a squeezed sliver.
- Reason: this is a visual/polish round like v0.3.7, not a structural one like v0.3.8 — no destinations, routes, or data flows changed, only how the existing nav/inputs/dialog render. The Taxi Driver Card's conditional removal clause did not trigger because fixing its one real bug (the Close-button squeeze) was cheaper than removing the whole feature, and removing shipped, real functionality (TextToSpeech-backed Speak, clipboard Copy) to save an already-small amount of code would have been a bigger, less reversible change than the operator's "if it affects progress" framing called for.

ADR-114: The bottom nav is now a `Box` overlay floating on top of full-screen content instead of a `Scaffold` `bottomBar` slot that reserves layout space; the Android manifest declares `resizeableActivity`/`android.max_aspect`; the web app's viewport meta is pinned to width-only.

- Background: the operator gave four instructions in one message: (1) add `android:resizeableActivity="true"` inside the Android manifest's `<application>` tag; (2) add an `android.max_aspect = 2.4` `<meta-data>` inside `<application>`; (3) don't hardcode pixel dimensions, use `match_parent`-equivalent sizing to fill the screen; (4) the web viewport shouldn't cap max height, only declare `width=device-width`. Instructions 1-3 use Android-specific terms (`AndroidManifest`, `match_parent`) and instruction 4 uses web-only terms (`viewport`, `width=device-width` is literal `<meta name="viewport">` syntax) — this repo is a monorepo with both an `android/` Kotlin/Compose module and the original Next.js web app (`app/`), so the two instructions were interpreted as targeting their respective platforms rather than being a single confused instruction. Separately, the operator also said the bottom nav "是浮现在page文字和内容上面的，不需要有边框，要融入" (floats on top of the page's text and content, doesn't need a border, should blend in) — a functional gap in v0.3.8/v0.3.9's implementation, where the nav bar only looked floating (rounded, inset) but was still structurally a `Scaffold` `bottomBar`, which reserves its full height as blank padding and prevents any content from ever actually rendering behind/under it.
- Decision (manifest): added `android:resizeableActivity="true"` to `AndroidManifest.xml`'s `<application>` element (exactly as specified — not duplicated onto `<activity>`, to stay scoped to what was asked) and an `<meta-data android:name="android.max_aspect" android:value="2.4" />` inside `<application>`, both supporting multi-window/split-screen and tall/foldable aspect ratios.
- Decision (Compose layout audit): searched the whole `android/app/src/main/kotlin` tree for hardcoded `.height(`/`.width(`/`.size(` calls with literal dp values — found only two, both legitimate small fixed-size UI elements already (`VisePandaBottomBar.kt`'s 52dp center-button spacer and 58dp Chat FAB circle), not screen-filling containers. Every top-level screen already uses `Modifier.fillMaxSize()`/`fillMaxWidth()`/`weight(1f)` (Compose's `match_parent` equivalent) — the codebase was already compliant with instruction 3; no code changes were needed for it, just confirmation.
- Decision (nav bar overlay): `VisePandaNavHost.kt`'s `VisePandaApp` no longer wraps `NavHost` in `Scaffold(bottomBar = VisePandaBottomBar(...))`. It now uses a `Box(Modifier.fillMaxSize())` with `NavHost` filling the whole box (`Modifier.fillMaxSize()`, no bottom padding) and `VisePandaBottomBar` layered on top via `Modifier.align(Alignment.BottomCenter)` (`VisePandaBottomBar` gained a `modifier: Modifier = Modifier` parameter for this). Screens that need their content to clear the bar's footprint add a new `Dimens.BottomNavContentClearance` (96dp — the bar's 64dp height + 16dp bottom inset + a small margin) as bottom padding: `TripsScreen`/`MeScreen`'s `LazyColumn` `contentPadding`, and `ButlerScreen`'s `ButlerComposer` (its input/camera/mic/send row, which — unlike scrollable list content — must never be visually or functionally obscured since it's the Chat tab's primary interactive control). `DayDetailScreen` needed no change: it's not a `TopLevelDestination`, so the nav bar was never shown there to begin with.
- Decision (web viewport): `app/layout.tsx` gained an explicit `export const viewport: Viewport = { width: "device-width" }` (Next.js 14+ App Router `Viewport` export), deliberately omitting `initialScale`/`maximumScale`/any height property — matching the operator's literal "只写 width=device-width" (only write width=device-width) rather than adding companion properties Next's default might otherwise imply. This is the first web-app (`app/`) touch since the project's `0.3.x` pivot to native Android/iOS (VERSIONING.md notes the web series stopped at `v0.2.17`) — scoped to this one-line, low-risk correctness fix, not a resumption of active web development.
- Verification: `./gradlew :app:testDebugUnitTest :app:assembleDebug` passes (one incremental-build dex conflict during iteration, resolved with `./gradlew clean` per the established non-code-bug pattern from v0.3.8). Manually verified on an Android 34 emulator: scrolling Trips all the way down shows its last day card resting with clear space above the floating bar (confirming content passes behind the bar mid-scroll, then clears it at rest); the Chat composer's input/camera/mic/send row sits fully above the bar with no overlap; Me/Explore/Tools render unaffected. `npx tsc --noEmit` on the web app shows zero errors introduced by the `layout.tsx` change (pre-existing, unrelated test-file type errors are untouched).
- Reason: instructions 1-3 and the nav-bar-overlay request are two sides of the same "make the app properly adapt to different screens/windows" concern — declaring aspect-ratio/resizability support in the manifest is meaningless if the Compose layer still assumes a fixed-height bottom chrome that eats into available content space; removing the `Scaffold`-reserved padding is what actually lets content use the full window on unusual aspect ratios/multi-window sizes, not just the manifest flags alone.

ADR-115: `ButlerScreen.kt`'s Chat composer is redesigned — suggestion chips move from a wrapping `FlowRow` to a horizontally-scrollable single-row `LazyRow`, and the input `OutlinedTextField` grows taller and wider by absorbing the Camera/Mic buttons as `leadingIcon`/`trailingIcon` instead of separate `Row` siblings.

- Background: after using the app across several verification passes, the operator gave direct feedback: "chat这一轮的布局要重新设计，输入框太小了，建议问题占比太大了" (this round, the Chat layout needs to be redesigned — the input box is too small, the suggested-questions area takes up too large a proportion). Both `EmptyButlerPrompt` (the welcome card shown before any message) and `ButlerComposer` (the persistent composer below messages) used a `FlowRow` for their suggestion chips, which wraps to a second line whenever chips don't fit one row's width — with 3 suggestions, this reliably wrapped, costing a full extra row of vertical space in both places. Meanwhile the input `OutlinedTextField` sat in a `Row` alongside two separate `IconButton`s (Camera, Mic) and a `Button` (Send), splitting the row's width four ways and capping the field at `minLines = 1`, so it read as a single thin line even though `maxLines = 4` let it grow once wrapped text forced it to.
- Decision (suggestions): both `EmptyButlerPrompt` and `ButlerComposer` changed their suggestion chips from `FlowRow(horizontalArrangement, verticalArrangement)` to `LazyRow(horizontalArrangement = Arrangement.spacedBy(Dimens.SpaceSM))` with `items(...)` — this guarantees exactly one row of height no matter how many suggestions exist; extra suggestions scroll horizontally instead of wrapping down. `ButlerComposer` also now skips rendering the row (and its spacer) entirely when `state.suggestions` is empty, rather than always reserving space for a row that might have zero chips. `ExperimentalLayoutApi`/`FlowRow` imports and the `@OptIn(ExperimentalLayoutApi::class)` annotations on both composables were removed since nothing in this file needs them anymore.
- Decision (input box): `OutlinedTextField` gained `leadingIcon = { IconButton(camera) }` and `trailingIcon = { IconButton(mic) }` (both still disabled visual-only placeholders per the existing point-of-use permission policy — unchanged behavior, just relocated) instead of those two `IconButton`s being separate items in the outer `Row`. The outer `Row` now only contains the `OutlinedTextField(weight = 1f)` and the `Send` `Button`, so the field claims nearly the whole row's width. `minLines` increased from 1 to 2 and `maxLines` from 4 to 6, so the field visibly presents as a multi-line box by default rather than growing only once wrapped text forces it. The `Row`'s `verticalAlignment` changed from `CenterVertically` to `Bottom` so `Send` stays pinned to the bottom-right as the field grows upward (the conventional chat-composer behavior), rather than re-centering against a taller field.
- Verification: `./gradlew :app:testDebugUnitTest :app:assembleDebug` passes. Manually verified on an Android 34 emulator: a cleared-data fresh launch shows the empty-state welcome card's suggestions as one scrollable row (not wrapped); the persistent composer's suggestion row scrolls horizontally (confirmed by swiping to reveal a chip that was off-screen); typing a long multi-word message grows the input box to multiple visible lines with Camera/Mic staying anchored inside the field and Send staying pinned to the bottom-right; no crashes.
- Reason: a wrapping `FlowRow` and a `LazyRow` look nearly identical with few, short chips, but diverge exactly at the case that was the actual complaint — 3+ chips wide enough to not fit one row. Capping suggestions to a single scrollable row is a stronger fix than shortening chip text or reducing padding, because it holds regardless of how many suggestions a future round adds. Moving Camera/Mic inside the text field as leading/trailing icons (a standard Material 3 `OutlinedTextField` affordance) is a more direct fix for "input box too small" than only raising `minLines`, since the previous layout's width-sharing was compounding the height problem — a field that's both narrow and short reads as much smaller than a field that's merely short.

ADR-116: The native Chat feature's real `/api/chat` calls were silently failing on every single request because OkHttp's default 10-second read timeout was shorter than the deployed orchestrator's real-world latency — this, not any request/response contract gap, was the actual reason Chat had only ever been observed running in `NativeButlerFallback` (offline) mode throughout this project's Android history. Fixed by raising client timeouts and adding a debug-only `HttpLoggingInterceptor`; separately closed the one real contract gap found (`AssistantResponse.toolCards`) and added the `InlineToolCard` model + rendering to reach parity with the web response shape.

- Background: the operator asked to "将web端已经配好的chat 的api接入，将网页端chat的功能和配置全部导入apk" (hook up the web side's already-configured chat API, and bring the web chat's functionality/configuration fully into the APK). Two research passes mapped both sides in full: the web `/api/chat` route (6 parallel-raced OpenAI-compatible providers, an 18s per-provider timeout, a deterministic mock fallback, an `ask_factual` fast path that returns `InlineToolCard`s without calling any LLM, and the full `CanvasPatch`/`AssistantResponse` contract) and the Android side (`ButlerApiService`/`RoomTripRepository`/`CanvasPatchApplier`/`NativeButlerFallback`, wired since v0.3.6). The request/response contracts matched almost exactly field-for-field — the only real gap was `AssistantResponse.toolCards`, which the Android `AssistantResponse` data class didn't have a property for at all (Gson silently drops unknown JSON fields on deserialize, so this wasn't a parse error, just silently-dropped data).
- Root-cause finding: a direct `curl` to the production `/api/chat` route succeeded and returned a real DeepSeek-generated response in ordinary testing, proving the backend itself is correctly configured with real provider keys. But a **timed** request against the same endpoint with a message that exercises the full mock trip payload took **14.4s**, and the same request replayed through the Android app (captured via a newly-added debug `HttpLoggingInterceptor`) took **20.2s** end-to-end. `di/AppModule.kt`'s `provideOkHttpClient()` used a bare `OkHttpClient.Builder().build()` — OkHttp's default connect/read/write timeouts are all 10s. Every prior manual verification pass in this project's history that showed the Chat screen (v0.3.6 onward) was, without exception, showing `NativeButlerFallback`'s "Saved for the Butler" copy — because every real request was silently aborted by a `SocketTimeoutException` before the server could ever respond, and `RoomTripRepository.sendButlerMessage()` wraps the Retrofit call in `runCatching`, converting that timeout into an unconditional, silent fallback with no visible error.
- Decision (timeout fix — the actual fix): `provideOkHttpClient()` now sets `connectTimeout(15s)`, `readTimeout(45s)`, `writeTimeout(15s)` — the 45s read timeout gives headroom above both the measured ~20s worst-case round trip and the orchestrator's own ~18s per-provider ceiling (a request could plausibly approach that ceiling if the fastest of several parallel providers is itself slow). Also adds a `HttpLoggingInterceptor` at `Level.BODY`, gated by `if (BuildConfig.DEBUG)`, so a future connectivity issue is diagnosable from logcat instead of requiring another multi-hour root-cause hunt.
- Decision (contract parity): added `InlineToolCard` (`id`, `categoryId`, `title`, `summary`, `items`, `nextAction`, `href`, `tone`, `sourceLabel`) and `InlineToolCardTone` (`Info`/`Warning`/`Success`) to `data/model/ButlerModels.kt`, and a `toolCards: List<InlineToolCard>? = null` field on `AssistantResponse`, mirroring the web's `lib/types/trip.ts` shape field-for-field. `ButlerScreen.kt`'s `MessageBubble` renders each tool card as a nested `Card` (title/summary/bulleted items/`nextAction` label) inside the assistant bubble. `href` deep-linking into a Tools category is deliberately NOT wired — `ui/tools/ToolsScreen.kt` is still an honest placeholder (lands with the native Translator round), so `nextAction` renders as plain labelled text, not a fake button that goes nowhere.
- Decision (deliberately NOT ported this round): `preferenceProfile` (the web's silent, regex-based dietary/budget/pace/interest extraction system, `lib/ai/preferenceProfile.ts`) was not built — it's a standalone subsystem, not a config value, and the field is optional on the request so its absence doesn't break anything; the `intent`/`strategy`/`providersTried`/`toolContext` response fields (used only for the web's own debugging status text) were not modeled since Android has no equivalent debug UI surface to show them in and Gson already ignores them harmlessly. Both are documented here as intentional scope decisions, not oversights, for a future round to pick up if ever needed.
- Verification: `./gradlew :app:testDebugUnitTest :app:assembleDebug` passes (one incremental-build artifact conflict during iteration — `bundleDebugClassesToRuntimeJar` reported a duplicate `R.class` entry — resolved with `./gradlew clean`, the same non-code issue class as the dex conflicts documented since v0.3.8). Manually verified end-to-end on a real, live Android 34 emulator against the production backend (not mocked): sent "What is the best way to see the Great Wall" against the full initial mock trip payload — request succeeded in 20169ms (previously would have hard-failed at 10s) and the app correctly rendered the server's own `mode: "mock"` graceful degradation response (distinct from `NativeButlerFallback`, confirmed by the differing copy and by `logcat` showing a real `200` response) after the server-side orchestrator's own three providers failed for that specific oversized-context request; separately sent "Do I need a visa for China" and got a real, fast (738ms) `ask_factual` tool-card response end-to-end, with the new `InlineToolCardView` correctly rendering the live server's `toolCards[0]` (title "Visa and entry", 4 bullet items, `tone: "warning"`).
- Reason: "接入" (hook up/integrate) implied the connection didn't already work, but the wiring from v0.3.6 was structurally correct — what was actually broken was invisible from the outside because the failure mode (a silent client-side timeout indistinguishable from "no network") looks identical to "backend down" or "no API keys configured," which is why this had gone unnoticed across every prior round's manual verification. Timed, direct backend testing (not just re-reading the existing code) was what surfaced the real cause — a lesson for any future "why does X always show the fallback" investigation: measure actual latency before assuming a code-logic bug.

ADR-119: Chatbot reliability + quality round (v0.3.17) — executed by the architect session as an operator-directed web-freeze exception, since the multi-agent split (Codex=iOS, Antigravity=Android) leaves the web/AI layer without an owner and the operator directly instructed "优化整个 chatbot".

- Background: the 2026-07-04 production connectivity audit found all three configured LLM providers unhealthy simultaneously — DeepSeek failing with "Unterminated string in JSON at position 5212" (completion truncated at max_tokens, then thrown away whole by a bare `JSON.parse`), Qwen timing out at the flat 18s ceiling, Zhipu returning HTTP 429 — with the orchestrator having no memory of failures, so every request re-raced the same sick providers and production chat ran 100% on the mock fallback while looking "fine" from the outside.
- Decision (truncation tolerance): new `lib/ai/jsonRepair.ts` — `extractJsonCandidate` (strip fences/chatter), `repairTruncatedJson` (close open string + bracket stack, null dangling keys, drop trailing commas/keys), `safeParseLlmJson` (raw parse → repair → backtrack to previous structural boundary, ≤24 steps). `parseButlerPatch` now goes through it. Salvaging the longest valid prefix beats discarding an entire itinerary because the model ran out of tokens mid-sentence.
- Decision (budgets): per-intent budgets replace the flat 2200 tokens / 18s. Itinerary intents (create_trip/adjust_trip/add_location/add_poi) get 4096 tokens / 25s — the system prompt contractually demands the COMPLETE days array, which for a 3-day trip regularly exceeds 2200 tokens, making truncation systematic, not incidental. Non-itinerary intents get 1400 / 15s so factual replies stay fast. Providers accept `timeoutMs` per call; `finishReason` is surfaced for observability.
- Decision (circuit breaker): in-memory per-provider breaker — 2 consecutive failures → skipped for 120s; success clears; if skipping would empty the candidate pool the breaker is ignored entirely (it can never lock chat into mock-only). Honest serverless caveat recorded: state is per warm instance and resets on cold start; the goal is avoiding repeated known-bad calls within an instance, not global health tracking (that belongs to butler-service, see docs/planning/butler-2.0-chatbot-design.md).
- Decision (quality gate): a create_trip patch with a missing/empty `days` array now throws in `parseButlerPatch` — a reply that claims to have planned a trip but didn't patch the canvas loses the race to a provider that did the work, instead of winning on speed and leaving the user with words but no itinerary.
- Decision (prompt constitution): `buildSystemPrompt` restructured into persona ("knowledgeable local friend") + six HARD RULES (never fabricate China facts — name an official source when unsure; bookings info-only; at most ONE clarifying question; reply language follows the user's message; distress-first ordering for danger/theft/lost-passport; accept corrections plainly) + the existing output contract + gap-driven suggestions guidance + length adaptivity. These were previously scattered or implicit; they are now versioned prompt law, and future edits should run the golden-conversation regression planned in Butler 2.0 Phase D.
- Decision (input slimming + sampling): `photoUrl` is stripped from the trip payload before prompting (display-only, long CDN URLs, zero planning value); jsonMode temperature defaults to 0.3 (structured contract wants tight sampling), free-text stays 0.4.
- Decision (distress fast-path): acute-distress vocabulary (robbed/stolen/theft/pickpocket/lost my passport|wallet|phone/injured/hurt/urgent help/call police/ambulance) added to the `concern` intent patterns and to the emergency category rules in `factualToolCards.ts`, so these messages deterministically surface the emergency tool card (110/120/embassy) via the no-LLM fast-path instead of falling through to `unclear` or a generic recommendation.
- Verification: 167 unit tests pass (18 new: 13 jsonRepair, 5 orchestrator — truncation recovery, quality gate, breaker skip, breaker never-locks — plus distress classification); `npm run build` clean. Breaker state exposed via `resetProviderHealthForTests()` to keep tests isolated.
- Reason: every fix targets a failure actually observed in production the same day, not a hypothetical. The round deliberately stays inside the existing TS orchestrator (no new infrastructure, no new deps) so it ships immediately on Vercel; the deeper multi-agent/memory work remains in the Butler 2.0 plan, which this round de-risks by making the fallback layer it will sit on genuinely reliable.

ADR-120: Chat/Butler AI 回复的失败契约改为"诚实报错"，不再用 mock Butler 编造假回答（操作者直接指令，2026-07-05）。

- Background: 部署 butler-service 验证时发现四个 Agent 全部无条件 mock（见 #27），操作者借此机会明确指令：Chat 只允许调用真实 LLM 对话，连接失败必须诚实显示"连接失败"，不允许再用编造的假行程/假回复冒充真实 AI 答案。
- Decision：`lib/ai/orchestrator.ts` 移除 `mockResult()`/`createMockButlerPatch` 兜底路径——无 provider 配置、全部 provider 失败、空消息校验，三处调用点全部改为抛出/返回失败信号，不再返回一个"看起来像真实 AI 生成"的 CanvasPatch。`app/api/chat/route.ts` 捕获失败后返回非 2xx（503）+ `{ok:false, error, message}`，不再对失败情况伪装成 200 成功。`components/chat/ButlerWorkspace.tsx` 同步移除前端自己的两处 mock 兜底（`body?.patch ?? createMockButlerPatch(...)` 和 catch 块里的 mock），改为展示诚实的"连接失败"状态，不修改画布。
- Decision（不受影响的范围）：Explore/Tools 的静态数据兜底、Translate 占位、客户端离线展示上次缓存——这些是"数据源降级到本地已知内容"，不是"伪造 AI 决策"，`AGENTS.md` 五条硬规则的第③条（mock/static fallback 不删除）继续适用于它们，只对 Chat 对话内容本身开例外。
- Decision（连锁修复）：Android 的 `RoomTripRepository.sendButlerMessage` 早已在真实失败时 `throw IllegalStateException`（不使用假 fallback），但这个"诚实失败"检测完全依赖 HTTP 层失败（网络异常/非 2xx）——过去 Web 端遇到"无 provider/全部 provider 失败"时仍返回 HTTP 200 + 假 CanvasPatch，这意味着 Android 会把假答案当成功答案正常渲染，从未真正触发它自己的诚实失败路径。本轮把 Web 端的失败改为真实非 2xx 状态码后，Android 现有代码不需要改，直接就能正确识别失败——这是一个"修好上游，下游自动受益"的案例。iOS 端的对应行为需要单独核实（未在本轮验证范围内，见 GitHub Issue 待建）。
- Reason：与项目一贯的"honest disclosure"原则一致——数据不确定时说不确定，AI 不可用时说不可用，都好过用一份看起来自信的假内容误导用户以为自己得到了真实建议。

ADR-121: 四家 LLM 真实连通性深化轮（架构师自派任务，操作者提供四把真实 key，2026-07-05）。

- Background: 操作者直接提供 DeepSeek/Kimi/Qwen/GLM 四把真实 API key，指令"深化对于 chat 的真实连通性修复优化"，由架构师自己给自己派活执行。四家真实 key 首次同时配置到本地环境（此前只验证过三家）。
- Decision（DeepSeek 混合推理）：实测 `deepseek-v4-flash` 与 Qwen/GLM 同属混合推理模型，默认开启思考，小 max_tokens 下会烧光预算返回空 content（`finish_reason:"length"`）。修复：`modelRegistry.ts` 给 DeepSeek 加 `extraBody:{thinking:{type:"disabled"}}`，与 GLM 同款禁用方式，验证无质量损失。
- Decision（Kimi 推理无法关闭 + 双重浮动下限）：实测 3 种参数组合（`thinking.disabled`/`enable_thinking:false`/`reasoning_effort:"minimal"`）均无法关闭 kimi-k2.6 的推理——有的报 400（暴露出该模型只接受 `temperature:1`），有的被静默忽略。同时发现 reasoning_content 与 content 共享同一个 `max_tokens` 上限，行程类预算（4096）有时不够推理耗尽后给 content 留空间，报错 "response did not include message content"。用真实系统提示词（含完整宪法+行程状态）实测，完整成功耗时在 61.5s 到超过 75s 之间波动，波动幅度远超玩具提示词测得的 7-33.5s。架构决策：不再和模型死磕，改为接受这个特性——`openaiCompatibleProvider.ts` 新增 `minMaxTokens` 机制（与既有 `minTimeoutMs` 同款：`Math.max(intent 预算, vendor 下限)`），Kimi 设 `minTimeoutMs:90000` + `minMaxTokens:8192`，留出真实测得延迟之上的余量。接受的代价：Kimi 在四选一竞速中永远排在 DeepSeek/Qwen/GLM（7-27s）之后，这个延迟平时不可见，只有在它成为最后候选时才会暴露——真实慢回答（或 ADR-120 的诚实超时报错）仍然好过假的秒回 mock。
...
ADR-123: Codex Harness 定位为"开发侧受约束只读检查",且 iOS 构建验证必须用确定性 CI 而非 AI（2026-08-21,操作者提供外部报告要求核查对比）。

- Background: 操作者提供了一份外部(GPT)撰写的 Codex Harness 集成方案,要求研究 harness 开源后能否加入 VisePanda 开发。核查发现该报告引用的基线 commit `f1f9de31…` 在本仓库不存在,其描述的 monorepo 结构(`packages/domain`/`apps/server`/`apps/ops`)与全部关键类型(`CopilotEnvelope`/`TripPatch`/`ExecutionFactSupport`/`CompletionJob`/Content AI)在本仓库 0 命中,ADR 与 Issue 编号体系也对不上——即它描述的不是本仓库。其关于 Codex 本身的技术判断经逐条核实基本准确,但所有具体落点不可执行。
- Decision（前提修正）：本项目已经是三 Agent 协作(`WORKFLOW.md`),**Codex 早就是 iOS 工程师**。因此"要不要引入 Codex"是个已经过时的问题;真正的问题是"harness 开源能否给已有的三个 AI 加一层确定性护栏"。
- Decision（P0,且明确不用 AI）：iOS 泳道**没有任何 CI**(仓库只有 `android-ci.yml` 与 `web-ci.yml`),`WORKFLOW.md` 原话"iOS 无 CI,截图是硬要求"。当前 iOS 的质量链条是"Codex 自己写→自己构建→自己截图→自己声明通过",全程没有独立于实现者的验证。正确解法是 GitHub Actions macOS runner 上的确定性 `xcodebuild`,**不是**再叠一层 AI preflight。
- Reason（方法论,本 ADR 的核心）：**在有确定性手段可用的地方引入 AI 判断,是把可证伪的事实降级成概率意见。** 一个 AI preflight 只能说"我读了 diff,看起来没问题";`xcodebuild` 能给出"编译通过/失败"这个不可辩驳的结果。外部报告的框架会引导团队去建 AI preflight,却没意识到这个项目缺的是最基础的编译验证——这是本次核查最有价值的分歧点。诚实记录代价:macOS runner 计费倍率显著高于 ubuntu,必须用 `paths:` 严格限定触发条件,操作者需要知晓成本影响。
- Decision（P1,harness 的首选用途）：**跨语言契约漂移检查**。同一份行程契约现在有三份实现——`lib/types/trip.ts`(TS)、`android/…/data/model/TripModels.kt`(Kotlin)、`ios/VisePandaIOS/Models/TripModels.swift`(Swift),`ButlerModels`/`TranslateModels`/`AuthModels` 等同样三份,由三个不同 AI 分别维护。ADR-094 已规定 Web↔Android 必须行为一致(连四舍五入都对齐),但没有任何自动化在检查这条规定,且 iOS 是后加的、没有等价 ADR 覆盖。`AGENTS.md` 已把"跨端一致的字段/命名契约"列为强制要求并记录 KMM 是"已知但未采纳"的长期选项,进一步印证这是真实痛点。
- Reason（为什么这项适合 AI 而 P0 不适合）：Kotlin data class / Swift struct / TypeScript interface 三向比对是**语义比对**而非文本比对(命名习惯、可选性表达、枚举写法都不同),linter 做不了;而输入是公开代码、输出是结构化字段差异列表、只读无副作用、可 schema 校验——正好落在受约束 AI 的强项区间。先本地手动验证准确率,再考虑以"PR 建议评论(不阻断合并)"形式进 CI。
- Decision（明确不做）：(1) 不把 harness 接入游客产品——与外部报告一致,理由是权限面远超旅行产品所需、会产生与 `CanvasPatch` 并行的第二状态系统、且 Vercel serverless 无常驻进程承载 app-server;(2) 不自动化操作者的三句话术 relay——当前工作流刚跑通且已出现过换人接手,人在环路里有价值,收益(省几次复制粘贴)远小于风险(失控的自动合并链);(3) PR preflight 优先级下调——`WORKFLOW.md` 已把 Claude Code 定为 Reviewer + 唯一合并权,再叠一层的边际收益低于补 iOS CI。
- Decision（app-server 边界,沿用并补强外部报告）：官方逐字标注 "Websocket transport is currently experimental and unsupported. Do not rely on it for production workloads."。补充根因:本项目 Web 端跑在 Vercel serverless 上,没有常驻进程可承载 app-server 的长连接与本地执行环境。
- Verification（诚实披露研究局限）：`developers.openai.com` 与 `learn.chatgpt.com` 均被本会话出站网络策略拦截(403,已用 `curl "$HTTPS_PROXY/__agentproxy/status"` 确认为策略拒绝,遵照代理说明未尝试绕过)。已核实项:`openai/codex` 为 Apache-2.0、三种集成方式、app-server WebSocket 实验性措辞(逐字)、`openai/codex-action` 只接受 provider API key 而 ChatGPT 订阅不可用、该 Action 自带 `permission-profile`/`safety-strategy`(默认 `drop-sudo`)。**未能核实项**:`codex exec` 的确切参数名(外部报告写的 `--ephemeral`/`--sandbox read-only`/`--output-schema`/`--ignore-user-config`)与 `sandbox_mode`/`approval_policy` 枚举值——落地前必须本地 `codex exec --help` 核对,不得照抄。
- Note（与官方口径的一处差异）：外部报告主张"模型 job 一律不得持有写权限",但 `openai/codex-action` 官方 README 的反馈类任务示例实际给了 `issues: write`/`pull-requests: write`。我们采纳更严格的版本,但记录清楚这是**本项目自定策略,非官方硬性要求**。
