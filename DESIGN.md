...
## v0.3.15 Design Update - Android Native Translator Implementation

ADR-117: Implement the native Translator utility with TTS offline fallback and a street-facing large dialog, while keeping camera/voice translations as disabled placeholders (Coming soon) to avoid requesting sensitive runtime permissions.

- **Background**: Under Issue #3, the mobile development main track requires building a native Translator utility in Compose. Key design issues to resolve: (1) Text translation needs to query Next.js proxy `/api/translate/text` in real-time, but must handle offline states elegantly; (2) The traveler needs survival phrases for common situations in China; (3) Foreigners on the street often struggle to speak to locals or let them read small text on a phone; (4) Camera scanning and Voice input are listed in the requirements, but real OCR/STT implementation would force immediate runtime permission requests (Camera & Microphone), which goes against the project's minimal permission policy for early alphas.
- **Decision (Data Layer)**: Created `TranslateRepository.kt` to define `TranslateApiService` and wrap the Retrofit instance. It executes POST queries against `/api/translate/text`. If a network exception is thrown, it returns a localized UI error ("Translation connection failed") rather than throwing a crash or freezing.
- **Decision (UI Layer & Phrase Book)**: Built `PhraseModel.kt` as a static, local survival dictionary containing 5 crucial travel scenarios (Essential, Shopping & Pay, Transport, Food, Hotel) with English, Chinese, and Pinyin translation.
- **Decision (Speech & Handoff)**: Integrated Android's built-in `TextToSpeech` (TTS) inside `TranslateViewModel.kt`, initialized safely via `@ApplicationContext`. When a traveler clicks a phrase, the Chinese voice is spoken off-line. When clicking a phrase card, a fullscreen/large-font `Dialog` (street-facing handoff dialog) is shown, displaying the Chinese characters in 32sp so it can be easily read by locals in noisy or low-light situations, alongside a giant "Speak" play button.
- **Decision (Testing affordance)**: Since `ToolsScreen.kt` (which Translator originally expands from) is currently a read-only placeholder outside our permitted Scope files, a temporary test button is layered directly inside `VisePandaNavHost.kt` on top of `ToolsScreen`. This button floats in the center, allowing the reviewer to transition directly to the native `TranslateScreen` with one tap on an emulator or debug device.
- **Reason**: This keeps the PR small and limits edits strictly to files defined in the issue Scope, adhering to `AGENTS.md` guidelines. Using standard `stringResource(R.string.translate_*)` ensures complete i18n parity. Placing Camera and Voice in a disabled, alpha-blended "Coming soon" state maintains design fidelity without introducing unneeded permission prompts, conforming to the "honest placeholder" rule.
