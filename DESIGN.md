...
ADR-117: Native translation component featuring Google ML Kit offline translation, TextToSpeech zh-CN pronunciation, travel-friendly Phrase Book dictionary, and street-helper fullscreen dialog.

- Background: The traveler needs a translation suite matching the Web Translator's three-tab capabilities (Text, Phrasebook, and street card dialog) built natively in Android with offline capacity for street situations where GFW or cellular signals block remote translation.
- Decision: Integrate Google ML Kit Translation library for offline translation, supporting dynamic download of zh-CN, English, and selected European model packs. Bundle Android TextToSpeech API for local voice pronunciation of translated texts. Implement an interactive survival phrase list categorized by scenario (Subway, Taxi, Restaurant, Shopping). Add a "Big Card Dialog" that renders translated phrases in large typography to easily show to locals on crowded streets.
- Verification: Compiled and verified on an Android 34 emulator; verified offline text translation works natively, survival phrases render cleanly, and TTS pronunciation outputs correctly.

ADR-118: Native Android Explore surface utilizing Next.js /api/explore/amap route, safe Gson JsonElement deserialization, automatic static fallback routing, and CanvasPatch integration via the Chat repository.

- Background: The traveler needs a native Explore board displaying curated attractions, food, and stays with foreigner-friendly recommendation Rationales, and a pipeline to seamlessly schedule them into their trip.
- Decision: Create `ExploreRepository` connecting to Next.js `/api/explore/amap` and falling back to a 100% matched local `MockExploreData` upon server or network failure. Model `AmapPoi` in Kotlin using `JsonElement` to safely parse dynamic `address`/`tel`/`biz_ext` payloads (which occasionally return empty arrays `[]` instead of expected objects). Add a `pendingPoi` pipeline in the shared `TripRepository` to inject the Explore POI payload into the next chat request. Directly dispatch the chat message from the Explore screen and slide the bottom nav to Butler (Chat) to display the real-time CanvasPatch result without modifying the Chat screen's signature.
- Verification: `./gradlew :app:assembleDebug` compile success; verify that clicking "Add to Trip" / "Save for Later" successfully triggers chat and inserts the POI into the Needs Scheduling section.
