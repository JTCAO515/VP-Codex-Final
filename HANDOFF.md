...
- 当前分支：`main`
- 当前版本：`v0.3.23`（版本序列 `0.3.x`；本轮是 Codex Harness 集成评估,纯研究,不改任何运行时代码/CI/依赖/key）
- ⚠️ **本文件存在已知漂移,不要当作完整历史**：下方"重要（已完成）"列表在 `v0.3.12` 与 `v0.3.23` 之间**缺失 v0.3.13–v0.3.22 全部条目**。`VERSIONING.md` 记录 v0.3.13(Tools 全量实现)/v0.3.14(Android APK 功能补全)确已在 `main` 落地(`fc519ab`…`4239bce`),其长文条目在一次操作者工作树回滚中被覆盖;v0.3.15–v0.3.22 同样未回填到本文件。**以 `CHANGELOG.md` + `VERSIONING.md` + git history 为准。**
- 重要（已完成）：
  - v0.3.23:**Codex Harness 集成评估(修订版)**。操作者提供一份外部(GPT)撰写的《VisePanda Codex Harness 集成研究与实施方案》,要求研究 harness 开源后能否加入本项目开发、如何优化产品,并对比后汇总重写。**核查发现该报告描述的不是本仓库**:它引用的基线 commit `f1f9de31…` 在本仓库不存在(`git cat-file -t` 无此对象),它描述的 monorepo 结构(`packages/domain`/`apps/server`/`apps/ops`)不存在,`CopilotEnvelope`/`TripPatch`/`expectedVersion`/`ExecutionFactSupport`/`CompletionJob`/QStash/Content AI **全部 0 命中**,ADR 编号是四位数(ADR-0022/0023/0024)而本仓库是三位数(ADR-070…ADR-122),Issue 引用 #496/#499-#508 而本仓库当前在 #150-#166。它关于 Codex 本身的技术判断经逐条核实**基本准确**(Apache-2.0、三种集成方式、app-server WebSocket 官方逐字标注实验性/不支持生产、`codex-action` 只接受 API key 而订阅不可用),但**所有具体落点不可执行**。修正后的三条结论:(1) **Codex 早已是本项目 iOS 工程师**(`WORKFLOW.md`),"要不要引入 Codex"是过时前提;(2) **发现 P0 漏洞:iOS 泳道没有任何 CI**(只有 `android-ci.yml`/`web-ci.yml`),当前 iOS 质量链条全程无独立于实现者的验证,而**正确解法是 macOS runner 上的确定性 `xcodebuild`,不是再叠一层 AI preflight**——在有确定性手段可用处引入 AI 判断,是把可证伪的事实降级成概率意见;(3) **harness 首选用途是跨语言契约漂移检查**——同一份行程契约现有 TS/Kotlin/Swift 三份实现由三个不同 AI 维护,ADR-094 要求 Web↔Android 行为一致却无任何自动检查,iOS 无等价 ADR 覆盖,而三向语义比对正是 linter 做不了、受约束只读 AI 擅长的。游客产品**绝不**接入 harness(与外部报告一致)。诚实披露:`developers.openai.com`/`learn.chatgpt.com` 被本会话出站策略拦截(403,已核实为策略拒绝,未绕过),`codex exec` 确切参数名未能核实,落地前须本地 `--help` 核对。详见 `docs/planning/v0.3.23-codex-harness-assessment.md` 与 DESIGN.md ADR-123。
  - v0.3.12:Chat 真实 API 根因修复 + toolCards 契约补全。操作者要求"将web端已经配好的chat 的api接入，将网页端chat的功能和配置全部导入apk"。两条并行调研线逐字段对比 Web 端 `/api/chat` 和 Android 端现状后发现:契约几乎完全对齐,真正的问题不是没接好——`RoomTripRepository`/`ButlerApiService` 自 v0.3.6 起就已经在调真实后端了,只是**每一次都在无声超时**。直接 `curl` 生产环境证明后端配置正常(能拿到真实 DeepSeek 回复),但计时后发现一次涉及完整行程数据的请求要 14.4 秒,通过新加的调试期 `HttpLoggingInterceptor` 在 App 里实测同一类请求要 20.2 秒——而 `di/AppModule.kt` 里的 `OkHttpClient` 用的是裸的默认值(10 秒超时)。这意味着整个项目历史上,每一次真实 chat 请求都在服务器还没来得及响应前就被客户端超时掐断,`runCatching` 把超时异常无声转换成了 `NativeButlerFallback` 的离线兜底文案——这正是为什么从 v0.3.6 到现在,手动验收看到的永远是"Saved for the Butler",从来没有一次是真实 AI 回复。修复:`provideOkHttpClient()` 显式设置 `connectTimeout(15s)`/`readTimeout(45s)`/`writeTimeout(15s)`,加 `HttpLoggingInterceptor`(仅 debug 启用)。顺带补上唯一真实的契约缺口:`AssistantResponse` 新增 `toolCards` 字段(新建 `InlineToolCard`/`InlineToolCardTone`),`ButlerScreen.kt` 新增 `InlineToolCardView` 渲染(`href` 深链到 Tools 分类页特意没接,因为 Tools 还是占位页)。顺手修复了放了好几轮没更新的 `versionCode`/`versionName`(1/"0.3.6" → 12/"0.3.12")。`./gradlew :app:testDebugUnitTest :app:assembleDebug` 通过(遇到一次已知的增量构建产物冲突,`clean` 后正常);**真实对接生产环境的端到端验收**(不是 mock):发送带完整行程数据的消息,请求真实耗时 20169ms(以前 10 秒就会被掐断),App 正确渲染服务器自己优雅降级的 `mode: "mock"` 响应;发送事实类签证问题,738 毫秒内拿到真实 `ask_factual` tool card 回复,`InlineToolCardView` 正确渲染服务器返回的卡片。详见 DESIGN.md ADR-116。
  - v0.3.11:Chat 输入区重新设计。操作者用过几轮之后反馈:"chat这一轮的布局要重新设计，输入框太小了，建议问题占比太大了"。改动:`ButlerScreen.kt` 里两处建议问题(空状态欢迎卡片 + 消息列表下方常驻的输入区)都从会换行的 `FlowRow` 改成单行可横滑的 `LazyRow`,无论建议数量多少都只占一行高度;`OutlinedTextField` 把相机/麦克风两个原本独立占一列的 `IconButton` 挪进了 `leadingIcon`/`trailingIcon`(功能不变,仍是禁用态占位),这样输入框本身几乎占满整行宽度,`minLines` 从 1 提到 2、`maxLines` 从 4 提到 6,默认就是一个看得出来的多行大输入框;发送按钮所在 `Row` 的垂直对齐从居中改成底部对齐,输入框长高时发送按钮始终钉在右下角。`./gradlew :app:testDebugUnitTest :app:assembleDebug` 通过;Android 34 模拟器清空数据后全新启动验收:两处建议问题都确认单行可横滑(用横滑手势验证能看到被裁切在屏幕外的建议),打长文字确认输入框跟着长高、相机/麦克风贴在输入框内部两端、发送按钮贴在右下角,全程无崩溃。详见 DESIGN.md ADR-115。
...
## v0.3.12 Handoff Update - Chat 真实 API 根因修复 + toolCards 契约补全

- 触发来源：操作者要求"将web端已经配好的chat 的api接入，将网页端chat的功能和配置全部导入apk"。
- **调研阶段**:并行派出两个调研任务,一个彻底梳理 Web 端 `/api/chat`(`app/api/chat/route.ts`)——6 个 OpenAI 兼容 provider(DeepSeek/Qwen/Zhipu/Moonshot/Ernie/MiniMax)并行竞速(`Promise.any`)、单 provider 18 秒超时、`lib/mock-ai/mockButler.ts` 确定性 mock 兜底、`lib/tools/factualToolCards.ts` 的 `ask_factual` 快速通道(签证/支付/汇率/地铁/eSIM/应急问题直接返回 `InlineToolCard`,完全不调用 LLM)、完整的 `CanvasPatch`/`AssistantResponse` 契约(`lib/types/trip.ts`);另一个梳理 Android 端现状——`ButlerApiService`/`RoomTripRepository`/`CanvasPatchApplier`/`NativeButlerFallback`(自 v0.3.6 起就已经在调真实后端,不是本轮才接上)。逐字段对比两份报告后确认:请求体(`message`/`trip`/`messages`,Android 缺少可选的 `preferenceProfile`,不影响功能)和响应体(`ok`/`mode`/`modelLabel`/`patch`/`suggestions`/`fallbackReason`,Android 额外缺少仅用于 Web 端自身调试文案的 `intent`/`strategy`/`providersTried`/`toolContext`)基本完全对齐;`CanvasPatch`/`TripSummaryPatch`/`ButlerAlert` 等核心字段逐一比对完全一致;唯一真实缺口是 `AssistantResponse.toolCards`——Android 的 `AssistantResponse` 数据类根本没有这个属性(Gson 反序列化时会静默丢弃 JSON 里的未知字段,所以不会报解析错误,只是这部分数据被悄悄丢掉,永远不会显示)。
- **真正的根因排查**:先用 `curl` 直接测生产环境 `https://www.go2china.space/api/chat`,拿到了真实的 DeepSeek 回复,证明后端本身配置完全正常(有真实 provider key)。但用 `time curl ...` 计时后发现:一次涉及较大上下文(完整 mock 行程 JSON)的请求耗时 **14.4 秒**。反过来看 Android 端:`di/AppModule.kt` 的 `provideOkHttpClient()` 一直是 `OkHttpClient.Builder().build()`,没有任何自定义超时——OkHttp 的默认 connect/read/write 超时全部是 **10 秒**。为了直接验证,给 OkHttpClient 加了一个调试期 `HttpLoggingInterceptor`,在真机模拟器上实测同一类真实请求,耗时 **20169 毫秒(20.2 秒)**——远超默认 10 秒超时。这意味着:**从 v0.3.6 到现在,这个项目历史上每一次真实的 chat 网络请求都在服务器还没来得及返回响应之前就被客户端超时机制掐断**了。`RoomTripRepository.sendButlerMessage()` 里 `runCatching { butlerApiService.chat(...) }` 会捕获这个 `SocketTimeoutException`,`remoteResult.getOrNull()` 返回 `null`,代码无声地转向 `NativeButlerFallback.createPatch(...)` 生成离线兜底响应——**这正是为什么整个项目历史上,每一次手动验收截图看到的 Chat 回复都是"Saved for the Butler"/"I saved this request locally..."的离线兜底文案,从来没有一次是真实 AI 生成的回复,即使代码结构上"已经接上"真实后端**。这个 bug 在外部表现上和"没接后端"/"没配 key"完全无法区分,所以一直没被之前的验收流程发现——直到这轮真正去计时测量真实延迟,而不是只重读已有代码逻辑,才找到根因。
- **修复(核心改动)**:`di/AppModule.kt` 的 `provideOkHttpClient()` 显式设置 `.connectTimeout(15, TimeUnit.SECONDS)`、`.readTimeout(45, TimeUnit.SECONDS)`、`.writeTimeout(15, TimeUnit.SECONDS)`——45 秒读超时在实测约 20 秒最坏情况和后端 orchestrator 自身约 18 秒单 provider 超时上限之上留了充分余量。同时加了 `HttpLoggingInterceptor`(`build.gradle.kts` 新增 `com.squareup.okhttp3:logging-interceptor:4.12.0` 依赖),仅在 `BuildConfig.DEBUG` 为真时启用、级别 `BODY`,这样往后再遇到连接问题可以直接看 logcat 里的完整请求/响应,不用再靠猜测和长时间根因排查。
- **修复(契约补全)**:`data/model/ButlerModels.kt` 新增 `InlineToolCard`(`id`/`categoryId`/`title`/`summary`/`items`/`nextAction`/`href`/`tone`/`sourceLabel`)数据类和 `InlineToolCardTone`(`Info`/`Warning`/`Success`)枚举,和 Web 端 `lib/types/trip.ts` 逐字段对齐;`AssistantResponse` 新增 `toolCards: List<InlineToolCard>? = null` 字段。`ui/butler/ButlerScreen.kt` 的 `MessageBubble` 新增 `InlineToolCardView` composable,把每张 tool card 渲染成消息气泡内嵌的卡片(标题/摘要/要点列表/`nextAction` 标签)。**`href` 深链到 Tools 分类页(如 `/tools?category=visa-and-entry`)特意没有接**——`ui/tools/ToolsScreen.kt` 目前仍是诚实占位页(要等 native Translator 轮才会实现真实内容),所以 `nextAction` 渲染成普通带颜色的文字标签而不是一个点了没反应的假按钮,遵循项目一贯的"不伪造可交互控件"原则。
- **明确决定不在本轮实现的部分(记录原因,不是遗漏)**:Web 端的 `preferenceProfile`(`lib/ai/preferenceProfile.ts`,一套基于正则的静默饮食/预算/节奏/兴趣提取系统)是一整套独立子系统而不是一个简单配置项,且该字段在请求里是可选的,不传不会导致任何功能异常,留给未来专门的偏好设置轮实现;响应体里的 `intent`/`strategy`/`providersTried`/`toolContext` 字段只是 Web 端自己用来生成调试状态文字(如"用 DeepSeek v4 flash 更新了 canvas"),Android 目前没有对应的调试 UI 展示场景,且 Gson 已经在无害地忽略这些多余字段,故未建模。
- **顺手修复**:`android/app/build.gradle.kts` 的 `versionCode`/`versionName` 自 v0.3.6 那一轮起就一直停留在 `1`/`"0.3.6"`,历经 v0.3.7-v0.3.11 都没有同步更新——这次一并修正为 `12`/`"0.3.12"`,和仓库 `package.json`/文档里的版本号对齐。
- **验证**:`./gradlew :app:testDebugUnitTest :app:assembleDebug` 过程中遇到一次已知的增量构建产物冲突(`bundleDebugClassesToRuntimeJar` 报重复的 `androidx/hilt/navigation/compose/R.class` 条目,和 v0.3.8 起记录的 dex 冲突属于同一类非代码问题),`./gradlew clean` 后重新编译通过。**真实对接生产环境的端到端手动验收(不是 mock,是真实网络请求)**:清空 App 数据全新启动,发送"What is the best way to see the Great Wall"(带完整初始 mock 行程数据作为上下文),通过 `adb logcat` 捕获的 `HttpLoggingInterceptor` 输出确认请求真实耗时 20169 毫秒、HTTP 200,App 正确渲染了服务器自己优雅降级返回的 `mode: "mock"`/`modelLabel: "mock fallback"` 响应内容("I kept the current route and noted this as planning context.")——这和 `NativeButlerFallback` 的离线兜底文案("Saved for the Butler")明显不同,证明这次是真实网络往返成功,只是服务器端三个 provider(Qwen 超时/DeepSeek JSON 截断/Zhipu 429 限流)对这个较大上下文请求都失败了,由服务器自己的 mock 兜底接管;另外发送"Do I need a visa for China",logcat 确认 738 毫秒内拿到 `mode: "tools"`/`strategy: "tool"` 的真实 `ask_factual` 响应,截图确认新写的 `InlineToolCardView` 正确渲染了服务器返回的"Visa and entry"卡片内容(4 条要点、`tone: "warning"` 对应的红色 nextAction 文字、"VisePanda Tools" sourceLabel)。
- 未改动任何导航路由、Room schema、UI 布局(除新增的 tool card 渲染)。这轮的核心价值是**发现并修复了一个此前一直存在但从未被诊断出来的真实 bug**,而不是新增功能——"chat 功能和配置全部导入 apk"这个请求本身假设了连接没接好,但实际的接线代码从 v0.3.6 起就是对的,真正缺的是能让这条连接在真实网络延迟下真正工作的超时配置。详见 DESIGN.md ADR-116。

## v0.3.11 Handoff Update - Chat 输入区重新设计

- 触发来源：操作者在实际用过几轮之后给出直接反馈:"chat这一轮的布局要重新设计，输入框太小了，建议问题占比太大了"。
- **建议问题:从会换行的 `FlowRow` 改为单行可横滑的 `LazyRow`**:`ui/butler/ButlerScreen.kt` 里有两处建议问题——`EmptyButlerPrompt`(还没有任何消息时显示的欢迎卡片)和 `ButlerComposer`(消息列表下方常驻的输入区)。两处之前都用 `FlowRow(horizontalArrangement, verticalArrangement)`,3 个建议 chip 大概率装不下一行宽度,会自动换到第二行,白白占用一整行竖直空间。改用 `LazyRow(horizontalArrangement = Arrangement.spacedBy(Dimens.SpaceSM))` + `items(...)` 后,无论建议数量多少永远只占一行高度,多出来的靠横向滑动查看。`ButlerComposer` 里还加了 `if (state.suggestions.isNotEmpty())` 判断,没有建议时完全不渲染这一行和它的 Spacer(以前哪怕是空的也会预留位置)。因为两处都不再需要 `FlowRow`,移除了 `ExperimentalLayoutApi`/`FlowRow` 的 import 和两个函数上的 `@OptIn(ExperimentalLayoutApi::class)` 标注。
- **输入框:相机/麦克风挪进 `OutlinedTextField` 内部,框本身明显变大**:之前的 `Row` 里装了 `OutlinedTextField(weight 1f)` + 相机 `IconButton` + 麦克风 `IconButton` + 发送 `Button` 四样东西平分宽度,输入框被压得很窄,而且 `minLines = 1`,平时看起来就是薄薄一条线(`maxLines = 4` 只在用户打字换行时才会体现出来)。改动:把相机图标改成 `OutlinedTextField` 的 `leadingIcon`、麦克风改成 `trailingIcon`(两者功能不变,仍是禁用态视觉占位,真实相机/麦克风权限仍留到 Translator 轮按需申请),这样外层 `Row` 只剩输入框和发送按钮两样东西,输入框几乎占满整行宽度。`minLines` 从 1 提到 2、`maxLines` 从 4 提到 6,默认就能看出是一个多行大输入框。`Row` 的 `verticalAlignment` 从 `CenterVertically` 改成 `Bottom`,这样输入框往上长高时,发送按钮始终钉在右下角,符合聊天输入区常见的交互习惯,而不是随着输入框变高被一起往下挤到中间。
- 验证：`./gradlew :app:testDebugUnitTest :app:assembleDebug` 通过。Android 34 模拟器手动验收:先 `adb shell pm clear` 清空 App 数据、全新启动,确保看到真正的空状态——欢迎卡片的建议问题确认是单行、不换行;用横滑手势验证常驻输入区下方的建议问题也是单行可横滑,滑动后能看到之前被裁切在屏幕外的第三个建议("Check my booking gaps");在输入框里打一段长文字("This is a longer test message to see how the input box grows"),确认输入框跟着长高到三行可见文字,相机图标始终贴在输入框内部左侧、麦克风贴在右侧,发送按钮始终贴在输入框右下角,全程无崩溃。
- 未改动导航路由/数据流/Supabase schema,也未改动相机/麦克风/发送的实际功能行为——纯粹是布局重排。详见 DESIGN.md ADR-115。
