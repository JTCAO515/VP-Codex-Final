...
- **⚠️ 最重要的一条**：本沙箱环境没有 Android SDK，且确认 `dl.google.com`（Gradle `google()` 仓库实际解析到的域名）被本会话出站网络策略拦截（`maven.google.com` 会 301 重定向到它，同样拿到 403；用 `curl "$HTTPS_PROXY/__agentproxy/status"` 核实是策略拦截记录而非临时故障，遵照代理使用说明"不要绕过/重试组织策略拒绝"没有尝试其他路径）。这意味着：
  - 从未执行过 `./gradlew` 任何任务，AGP 插件解析这一步就会失败，更谈不上依赖解析、Compose 编译、KSP 注解处理或打包。
  - Gradle wrapper 二进制（`gradle-wrapper.jar`）没有生成——本地安装的 Gradle 8.14.3 没有可复制的 wrapper jar，且 `gradle wrapper` 任务本身在项目配置阶段就因为插件解析失败而报错。`android/README.md` 写明了拿到网络环境后第一步要跑什么命令。
  - 唯一能用的本地工具是 `apt` 装的 `kotlinc`（1.3.31，非常旧，不支持 Kotlin 2.0 惯用的尾随逗号语法），验证时把它判定为不可靠信号，没有采纳它的报错作为代码问题的依据；全部代码改为逐文件人工复核语法与 API 用法。
  - 所有版本号（AGP/Kotlin/Compose BOM/Hilt/Room/Navigation Compose）是撰写时基于训练知识的合理起点，不是已验证可用的组合，`android/README.md` 明确写了"当作起点核实，不是保证"。
- 下一位接手者（无论是人还是有 SDK/网络权限的 coding agent）**第一件事必须是跑一次真实的 `./gradlew :app:assembleDebug`**，把 README 里的披露换成真实的通过/失败记录，再继续写代码。已经为此写了一份可以直接交给 Codex 的接手提示词（见会话记录，或要求当前 agent 重新生成）。
