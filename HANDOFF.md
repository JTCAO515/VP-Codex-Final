...
- 当前分支：`agent/antigravity-android-explore`
- 当前版本：`v0.3.16`（版本序列 `0.3.x` 原生 Android APK 主干）
- 重要（已完成）：
  - v0.3.16: Explore 真实数据源与 CanvasPatch 候选队列连通。对接 Next.js `/api/explore/amap` 接口，新设 `ExploreRepository` 进行 live 状态探针检测与 staticCurated 降级回落；重构 POI Gson 安全防崩溃解析，解决 empty array 回落异常；在 `TripRepository` 中加入 pending POI 通道，打通 Add to Trip 和 Save for Later 到行程 Canvas 的 `time: "Flexible"` 待排定候选队列（不直接强改行程，经过 AI CanvasPatch 并由 `applyExplorePoiToPatch` Kotlin 版算法离线自闭环富化）；全新绘制 ExploreScreen 视图（横滑城市选择栏、3-Tab Attractions/Food/Stays、置信度与 Fit Rationale 显示、Add/Save 动作）。详见 DESIGN.md ADR-118。
  - v0.3.15: 原生翻译组件交付。交付 ML Kit 离线文本翻译、Phrase Book 场景生存词典、TTS 发音朗读与大字报大字展示 Dialog 弹窗。详见 DESIGN.md ADR-117。
