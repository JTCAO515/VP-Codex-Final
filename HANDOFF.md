...
- [ ] 在 Vercel 配置 `DEEPSEEK_API_KEY`，确认生产环境走 `deepseek` mode。
...
- DeepSeek 输出虽要求 JSON，但真实模型仍可能返回无效 patch；当前会自动回落到 mock。
...
1. 在 Vercel 项目中配置 `DEEPSEEK_API_KEY`，部署后用生产域名验证 chat mode。
2. 继续打磨桌面横屏 Chat / Trip Canvas 的信息密度和抽屉体验。
3. 为移动竖屏端单独设计抽屉/画布适配。
