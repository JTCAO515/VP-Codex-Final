...
- [x] 任务 5.2：让管家提醒可以深链到对应工具（轻量列表 + `/tools?category=<id>`，不恢复顶部任务卡）。
...
- Butler reminders 深链约束（`v0.1.19`）：`components/canvas/ButlerReminders.tsx` 渲染在 `TripCanvas` 行程时间线下方（不在 Canvas 顶部，不是固定 5 张任务卡），按 `ButlerAlert.type` 映射到对应 Tools 分类 id（`visa`→`visa-and-entry`、`payment`→`payment-setup`、`language`→`translate`、`transport`→`metro`、`risk`/`emergency`→`emergency`），点击跳转 `/tools?category=<id>`；没有映射的类型（`booking`、`weather`）只显示文字，不渲染链接。`components/tools/ToolsBoard.tsx` 挂载时读取该参数并预选中对应分类。不允许恢复 `CanvasTaskStrip` 顶部任务卡渲染。
...
- M6.5：管家提醒深链到对应 Tools 分类完成（2026-06-29，v0.1.19）。
- M7：目的地感知背景切换完成（待排期）。
