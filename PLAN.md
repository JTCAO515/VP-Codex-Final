...
- [x] `v0.3.9` 导航栏圆角悬浮化 + 文本框/对话框加大圆角(操作者临时插入的一轮,见下方"v0.3.9 附录"):
  - 悬浮胶囊导航栏(四周留白、`Ink` 深色背景、图标无文字标签)、聊天输入框与 Taxi Card 对话框加大圆角。
  - 意外发现并修复 Taxi Card 对话框 Close 按钮被挤压的布局 bug(详见附录)。
- [x] `v0.3.10` 屏幕适配打磨(操作者临时插入的一轮,见下方"v0.3.10 附录"):
...
## v0.3.7 附录 —— Android 视觉层对齐 Figma Make 设计参考(代码已产出并已真实验证)

- [x] **触发与范围界定**:操作者提供 Figma Make 文件 `Design According to MD Document`(https://www.figma.com/make/8J1WnuwCHwx60bSC6f7fQn/),要求"将布局修改为figma的设计"。该文件的产品结构(五屏 Chat/Plan/Explore/Tools/**Me**、两侧+中间悬浮按钮底部导航)与 repo 既有路线图(Today/Chat/Plan/Explore/Tools 横向导航)有实质性差异,已用 `AskUserQuestion` 征询操作者采纳范围,操作者选择"只做视觉层",产品结构和路线图节奏均不变。
- [x] **读取方式**:Figma MCP 对 Make 文件的 `get_design_context` 默认只返回 resource_link 清单不内联代码;改用浏览器打开该 Make 文件、切到内置代码编辑器视图,用页面 JS 访问 CodeMirror 6 的 `EditorView.state.doc` 取出完整 `App.tsx` 源码,抓取配色/字体/圆角/字号等视觉 token。
- [x] **配色**(`ui/theme/Color.kt`):对齐 Figma 精确值——纸白 `#FAF8F4`、中国红 `#C1292E`、金色 `#C9A84C` 等。这是对 ADR-094/095"和 Web 端 `app/globals.css` 逐字对齐"规则的一次刻意例外,只改 Android 未同步改 Web,记录见 DESIGN.md ADR-105。
- [x] **字体**(`ui/theme/Type.kt`):首次引入自定义字体——下载 Google Fonts 的 Playfair Display(标题)+ DM Sans(正文/UI),均为 variable font(OFL 许可可自由打包),通过 `FontVariation.Settings` 按字重取用,替换掉 v0.3.3"系统字体占位,自定义字体推迟"的决定。
- [x] **圆角**(`ui/theme/Dimens.kt`/`Theme.kt`):新增 `RadiusXL`(20dp)/`RadiusPill`(999dp),`Shapes.medium/large` 调大,卡片呈现 Figma 要求的"更软"观感。
...
- [x] **触发与范围界定**:操作者提供一整套黑色手机 mockup 产品设计参考图(5屏:Chat home/Plan/Explore/Tools/Me),给出三条直接指令:(1)"导航栏要做圆角悬浮式";(2)"taxi driver card 不重要 优先级极低，如果影响了项目进展可以移除";(3)"所有的文本框，对话框都做成圆角的，弧形"。这是一轮视觉打磨,不涉及导航结构/路由/数据流改动,范围类似 v0.3.7 而不是 v0.3.8。
- [x] **导航栏**(`navigation/VisePandaBottomBar.kt`):外层 `Surface` 从贴边 `fillMaxWidth().height(80dp)` + 纸白色背景,改为 `Box` 包水平/底部各 16dp `padding`(悬浮留白)+ `RoundedCornerShape(Dimens.RadiusPill)` + `Ink`(`0xFF1C1410`)深色背景,高度收窄到新增的 `BottomNavFloatingHeight`(64dp)。两侧 `SideNavItem`(Trips/Explore/Tools/Me)去掉文字标签只保留图标,未选中态图标颜色从 `onSurfaceVariant` 改为 `Paper.copy(alpha = 0.55f)`(半透明纸白,保证深色底上的对比度),选中态仍是 `colorScheme.primary`(Cinnabar 红)。中间悬浮圆形 Chat 按钮形状/颜色/交互不变,重新对齐到更矮的新导航栏。`ui/theme/Dimens.kt` 新增 `BottomNavFloatingHeight`(64dp)/`BottomNavHorizontalInset`(16dp)/`BottomNavBottomInset`(16dp)三个常量。
- [x] **文本框/对话框圆角**:`ui/butler/ButlerScreen.kt` 聊天输入框 `OutlinedTextField` 加 `shape = RoundedCornerShape(Dimens.RadiusXL)`(20dp,复用既有 token);`ui/components/TaxiDriverCard.kt` 的 `AlertDialog` 同样加 `shape = RoundedCornerShape(Dimens.RadiusXL)`。
...
- [x] **触发与范围界定**:操作者一条消息给出四条技术指令,混合了 Android 端和 Web 端术语:(1)"AndroidManifest application 标签内加入 resizeableActivity="true"";(2)"application 内部加入 max_aspect=2.4 元数据";(3)"不要写死任何像素尺寸，全部用 match_parent 铺满屏幕";(4)"viewport 不要限制最大高度，只写 width=device-width"。前三条是 Android 专属术语(`AndroidManifest`/`match_parent`),第四条"viewport"/"width=device-width"是 HTML `<meta name="viewport">` 的字面语法,只在 Web 语境下有意义——本仓库是 monorepo,`android/` 是原生 Android 模块,`app/` 是原 Next.js Web 端(自 v0.2.17 起冻结,主线转向原生),因此按平台边界把前三条落在 Android、第四条落在 Web。另外操作者单独指出:"导航栏是浮现在page文字和内容上面的，不需要有边框，要融入"——这是对 v0.3.9 的功能性补完。
- [x] **AndroidManifest**:`<application>` 加 `android:resizeableActivity="true"`(严格只加在 `<application>`,未重复加到 `<activity>`,避免超出操作者原话范围);`<application>` 内新增 `<meta-data android:name="android.max_aspect" android:value="2.4" />`。两者配合支持分屏/多窗口和超高屏/折叠屏宽高比。
- [x] **Compose 布局审计**(第 3 条指令):全仓库 `grep` `.height(`/`.width(`/`.size(` 硬编码调用,只发现两处合理的小尺寸 UI 元素(`VisePandaBottomBar.kt` 的 52dp 占位 Spacer、58dp Chat 悬浮圆按钮),不是容器级尺寸。所有顶层页面已统一用 `fillMaxSize()`/`fillMaxWidth()`/`weight(1f)`——代码库本来就合规,无需改动,只做确认并记录审计方法供未来复用。
...
- [x] **触发与范围界定**:操作者在实际用过几轮之后给出直接反馈:"chat这一轮的布局要重新设计，输入框太小了，建议问题占比太大了"。这是一轮纯 UI 布局调整,只涉及 `ui/butler/ButlerScreen.kt` 一个文件,不涉及导航路由、数据流或 Supabase schema。
- [x] **建议问题从 `FlowRow` 改为 `LazyRow`**:`EmptyButlerPrompt`(空状态欢迎卡片)和 `ButlerComposer`(消息列表下方常驻输入区)两处建议问题都从 `FlowRow(horizontalArrangement, verticalArrangement)` 改为 `LazyRow(horizontalArrangement = Arrangement.spacedBy(Dimens.SpaceSM))` + `items(...)`——3 个建议 chip 用 `FlowRow` 大概率装不下一行会自动换行,`LazyRow` 保证永远只占一行高度,多出来的靠横向滑动查看。`ButlerComposer` 额外加了 `if (state.suggestions.isNotEmpty())` 判断,没有建议时完全跳过这一行和它的 `Spacer`。因两处都不再需要 `FlowRow`,移除了 `ExperimentalLayoutApi`/`FlowRow` 的 import 和两个函数上的 `@OptIn(ExperimentalLayoutApi::class)` 标注。
...
## v0.3.23 附录 —— 产品定位对抗性评审与修订(纯文档,已完成)

- [x] 操作者上传《VisePanda 产品规划建议与项目实现路径书》,要求引入 DeepSeek/Kimi/Qwen/GLM 四个外部模型做对抗性评审。实际执行发现四个模型域名被本环境出站网络策略屏蔽(403 策略拒绝,不可绕过),已如实报告;操作者确认"先用可以用的方式跑一轮",遂由架构师本人完成两轮自我对抗评审(五角度独立批判 + 反诘收敛),完整报告见 `docs/planning/v0.3.23-product-positioning-adversarial-synthesis.md`。
- [x] 核心结论:规划书对项目现状理解已过时(仓库早已转向 Android 主线 + iOS 对齐,规划书大量"待建"能力已实现);规划书"Explore 应降级为纯动作层"的主张与团队近期投入的 Explore 大众点评式发现层工程正面冲突,不采纳。
- [x] 一句话定位修订为"救急内核 + 可信发现层"的中国旅行随身管家。
- [ ] **后续排期(尚未开工,留给下一轮实现)**:离线可用能力最小可行范围 —— 断网时展示今日行程 / 酒店地址 / 常用短语 / 紧急联系人 / 已生成的 Show to Local 地址卡,全部是已存在于本地状态里的数据,只需暴露成明确的离线可用状态,不需要新建离线存储架构。Android/iOS 各自拆一个 Issue。
- [ ] **后续排期(尚未开工)**:`Me` 页订阅 placeholder 计费单位调整 —— `Human Service` 从"$9.99/月"改为按次人工任务计费,`Premium Service` 从"$19.99/月"改为 Trip Pass(7天/14天一次性通行证),不新增命名体系,不接入真实支付(仍卡在 App Store Connect 商品 ID 未定)。
- [ ] **后续排期(尚未开工)**:"5 分钟现场任务完成率"埋点 —— 规划书提出但仓库里目前没有对应埋点代码,是本轮唯一"规划书说了、代码里查无此功能"的关键缺口。
- [x] 未改动任何运行时代码、API 契约、Supabase schema 或客户端实现。
