...
- 不做真实支付。
- 相机/麦克风权限：自 2026-07 起解禁（操作者授权，OCR/语音翻译场景），但仅限 Issue 明确要求的场景（当前仅 Translator 的拍照/语音翻译），不得顺手扩展到其它功能。任何危险权限（相机/麦克风/位置等）必须遵守：
  - **按需申请**：只在用户主动点击对应入口（"拍照翻译"/"语音翻译"按钮）时才弹权限请求，禁止 App 启动时预先申请。
  - **拒绝后优雅降级**：显示诚实提示（说明需要什么权限、去哪开），不崩溃、不反复弹框騒扰、不假装功能可用。
  - **一次一权限**：不打包多个权限一次性申请。
