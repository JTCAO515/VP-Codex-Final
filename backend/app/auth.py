...
def _verify_supabase_jwt(token: str) -> dict:
    # Test bypass: ONLY for pytest/local testing.
    # Enabled only when AUTH_TEST_BYPASS=1 and token format is "test:<user_id>".
    if token.startswith("test:") and os.getenv("AUTH_TEST_BYPASS") == "1":
        user_id = token.split(":", 1)[1].strip() or "test_user"
        return {"sub": user_id}
