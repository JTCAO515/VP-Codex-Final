...
from app.models import EventLog, Quote, RFP, ServiceOrder, Supplier, Trip
...
@router.post("/rfps")
def rfp_create(payload: RfpCreateIn, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        # trip must exist and belong to user
        trip = db.query(Trip).filter(Trip.id == payload.trip_id, Trip.user_id == principal.user_id).one_or_none()
        if not trip:
...
def rfp_get(rfp_id: str, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        rfp = db.query(RFP).filter(RFP.id == rfp_id, RFP.user_id == principal.user_id).one_or_none()
        if not rfp:
...
def service_order_create(payload: ServiceOrderCreateIn, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        ...
        rfp = db.query(RFP).filter(RFP.id == payload.rfp_id, RFP.user_id == principal.user_id).one_or_none()
        if not rfp:
            raise HTTPException(status_code=404, detail="RFP not found")
...
def service_order_get(order_id: str, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        o = db.query(ServiceOrder).filter(ServiceOrder.id == order_id).one_or_none()
        if not o:
            raise HTTPException(status_code=404, detail="Order not found")
        rfp = db.query(RFP).filter(RFP.id == o.rfp_id, RFP.user_id == principal.user_id).one_or_none()
        if not rfp:
            raise HTTPException(status_code=404, detail="Order not found")
        return {
            "order": {
...
def rfp_shortlist(rfp_id: str, payload: RfpShortlistIn, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        rfp = db.query(RFP).filter(RFP.id == rfp_id, RFP.user_id == principal.user_id).one_or_none()
        if not rfp:
