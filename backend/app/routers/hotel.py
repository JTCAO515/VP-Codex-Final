...
def hotel_book(payload: HotelBookingIn, request: Request):
...
def hotel_booking_get(booking_id: str, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        b = db.query(HotelBooking).filter(HotelBooking.id == booking_id).one_or_none()
        if not b:
            raise HTTPException(status_code=404, detail="Booking not found")
        trip = db.query(Trip).filter(Trip.id == b.trip_id).one_or_none()
...
def hotel_booking_cancel(booking_id: str, request: Request):
    with session_scope() as db:
        principal = get_principal(request, guest_id=None)
        b = db.query(HotelBooking).filter(HotelBooking.id == booking_id).one_or_none()
        if not b:
