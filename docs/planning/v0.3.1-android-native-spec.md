...
1. **MapView 的 Recomposition 性能崩溃漏洞**：
   ...
   - *质问*：Android 13+ 对媒体与定位权限（包括相册读取、相机使用、精确定位）有严格的分级声明。方案中提到“自动获取高德地图定位和照相”，如果没有处理权限动态拒绝、只读模式降级，直接启动高德定位 SDK 会导致安全策略中断直接杀进程。
...
*   **打车卡片显性入口直达**：在 Today、Canvas/Plan 当前行程卡与 Day 详情页提供清晰的 “Show Taxi Driver” 按钮，点击后直接弹出当前景点或酒店的中文司机沟通卡。该入口必须可见、可解释、可撤销，不依赖隐藏手势、硬件按键或后台常驻触发。
...
- **页面 4: Tools Screen (离线与焦虑消除)**
  ...
  - 打车卡（Show Taxi Driver）通过 Today 快捷按钮、Canvas/Plan 当前行程卡和 Day 详情页显性按钮全屏拉起，展示大字中文、拼音以及高德的截屏路径。
...
└── 4. Tools (工具小件 - 焦虑消除)
    ...
    └── 司机中文卡片 (Show Taxi Driver - 显性按钮拉起与大字模式)
```
...
- **Day 详情**:不用阶段 1～4 里提到的 `ModalBottomSheet` 承载(内容量偏大,
  ...
  问题的直接回应之一。v0.3.2 已进一步修正:不采用隐藏手势、硬件按键或后台常驻触发,
  改为 Today / 当前行程卡 / Day 详情页的显性按钮 + 复制按钮,确保入口可见、可解释。
