...
- **生产管线**(2026-07-05 操作者提出爬虫方向后升级为"合规爬取 + LLM 结构化 + 高德查证"三段式):
  1. **合规爬取层**(自动化采集,只碰法律干净的源):
     ...
     - **公开榜单类事实**(操作者点名:黑珍珠/米其林/本地人打卡榜):黑珍珠餐厅指南公开名单、米其林指南公开名单、文旅局 A 级景区名录、必吃榜/本地打卡榜公开报道——场所名称/入选事实不受版权保护,作为"热门信号"入库(只取名字和榜单事实,不抄任何评价文本)。入选榜单写进 `list_badges[]` 字段(如 `black-pearl-2026`/`michelin-1-star`/`local-favorite`),商户卡片渲染成角标——这是比评分更强的可信度信号,也是合法拿到"点评级"权威背书的方式
...
- **Skill 化**(操作者指令,2026-07-05):整套采集管线封装为项目级 Claude Code skill `curate-city-pois`(`.claude/skills/curate-city-pois/SKILL.md`),按城市为单位可重复调用(`/curate-city-pois chengdu`),内含榜单信号采集 → 结构化 → 高德查证 → 生成 Supabase seed 的完整流程和合规红线。**城市扩张路线**:Phase 1 热门三城(北京/上海/成都)→ Phase 2 其余六城(西安/广州/杭州/苏州/重庆/南京)→ Phase 3 九城之外的新城市(同步扩 `AMAP_CITY_MAP` 与城市选择器)。每扩一城 = 跑一次 skill + 架构师抽验入库,边际成本递减
