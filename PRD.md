...
### 2026-07-06 新增（商业化合规边界，源自四模型对抗性评审识别出的合规暗坑）

- **不做代付**：不做任何"用户以外卡付款、VisePanda 用人民币结算给境内商户"性质的功能，涉及跨境资金流，合规风险不可接受。
- **不做外币兑换/多币种钱包**：不让用户在 App 内存入非人民币资金再由 VisePanda 结算，这属于外汇业务，需要国家外汇管理局牌照。
- **不做"先收款后结算给商户/服务方"的资金二次清算（二清）模式**：无证二清是央行重点打击对象。
- **不做无证的旅行社业务性质的交易闭环**：代订酒店/机票/门票/导游服务如果涉及收取服务费或差价，可能被认定为需要《旅行社业务经营许可证》的旅行社业务，第一阶段不做。
- **定制化人工服务方向的商业化，履约方案不清楚之前不排期实现**：只有"信任漏斗"这个方向判断，没有具体服务形态、履约方、责任边界之前，不视为可执行的产品需求。
...
## v0.3.9 需求更新 —— 导航栏圆角悬浮化 + 文本框/对话框加大圆角(已完成)

需求来源:操作者提供一整套黑色手机 mockup 产品设计参考图(5屏:Chat home/Plan/Explore/Tools/Me),给出三条直接指令:(1)"导航栏要做圆角悬浮式";(2)"taxi driver card 不重要 优先级极低，如果影响了项目进展可以移除";(3)"所有的文本框，对话框都做成圆角的，弧形"。这是纯视觉打磨轮,不涉及导航结构/路由/数据流改动。

交付:底部导航从贴边、纸白色、80dp 高的横条,改为四周留白 16dp、`RadiusPill` 圆角、`Ink` 深色背景、64dp 高的悬浮胶囊;两侧图标去掉文字标签,未选中态用半透明纸白色保证深色底上的对比度;聊天输入框与 Taxi Driver Card 对话框都加大圆角(`RadiusXL`,20dp)。

验收标准:`./gradlew :app:assembleDebug` 通过;Android 34 模拟器手动验收悬浮胶囊导航栏在 Trips/Explore/Chat/Tools/Me 五个页面都渲染一致、选中态高亮正确,输入框/对话框圆角渲染正确,全程无崩溃。
