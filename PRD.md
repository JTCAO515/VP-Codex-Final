...
| Translator 页面 (Android) | P1 | 独立翻译页面（Android 原生翻译模块已在 v0.3.15 落地），含文字翻译（防抖）、本地离线 TTS 朗读与常用短语词典（PhraseBook）。 | 用户打开翻译器能看到文字翻译与短语词典 Tab。文字翻译支持 EN↔ZH 双向并对接 `/api/translate/text` API 代理服务，断网时优雅容错提示；短语词典本地包含 5 类实用生存短语（Essential/Shopping/Transport/Food/Hotel），每条带 TTS 离线播报。点击短语卡片弹出 32sp 大字街头出示 Dialog 方便游客向本地人出示并配有 giant 朗读键；相机 OCR 扫描与语音 STT 翻译均以 Alpha 灰度 Coming soon 禁用卡占位，不提前索求敏感系统权限。 |
