...
## v0.3.4 Agent Update - First Android Code Landed; Build Was Never Verified

- `android/` now contains real Kotlin/Gradle source (v0.3.3 Native Foundation + v0.3.4 Today/Plan Execution MVP, delivered together). **Read `android/README.md` before touching this directory** — it carries a load-bearing disclosure: this code has never been run through `./gradlew`, because the authoring sandbox had no Android SDK and `dl.google.com` (Gradle's `google()` repository host) was blocked by the session's egress policy. Do not assume it compiles. Your first action in this directory should be a real build, not a new feature.
- If you have SDK/network access and the build fails: that is expected and useful information, not a sign something went catastrophically wrong. Fix the actual compiler errors, update `android/README.md`'s disclosure once you've confirmed a real pass/fail, and keep moving — don't rewrite the whole module from scratch on the assumption it's unsalvageable.
...
- Needs Scheduling / candidate-state management for Plan is intentionally not implemented yet — it's grouped with v0.3.7 (Explore + Candidate Pipeline) so the UI state machine is designed against a real candidate source, not invented twice.
