...
- Needs Scheduling / candidate-state management for Plan is intentionally not implemented yet — it's grouped with v0.3.8 (Explore + Candidate Pipeline, renumbered from v0.3.7 after v0.3.6 consumed the Butler slot) so the UI state machine is designed against a real candidate source, not invented twice.
...
- Tools/Explore placeholder copy in `strings.xml` was renumbered again in v0.3.7 (now v0.3.8/v0.3.9) since that round consumed v0.3.7 — keep these two in sync whenever a version renumbering happens again.
...
- `strings.xml`'s Tools/Explore placeholder copy is now v0.3.9/v0.3.10 (renumbered again since this round consumed v0.3.8) — keep these in sync on any future renumbering.
