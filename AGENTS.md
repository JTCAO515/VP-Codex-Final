...
- 不要重新加入独立 Practical Reminder / Butler Rail 区块；管家提醒后续应进入 Tools 或更轻量的上下文提示，不占据 Canvas 顶部。`v0.1.19` 起 `components/canvas/ButlerReminders.tsx` 渲染在行程时间线下方，按 `ButlerAlert.type` 深链到对应 `/tools?category=<id>` 分类，不允许把它做成恢复的顶部任务卡或固定 5 项布局。
