...
- Needs Scheduling / candidate-state management for Plan is intentionally not implemented yet — it's grouped with v0.3.11 (Explore + Candidate Pipeline, renumbered several times as v0.3.7/v0.3.8/v0.3.9 each got consumed by an inserted operator-directed round) so the UI state machine is designed against a real candidate source, not invented twice.
...
- Tools/Explore placeholder copy in `strings.xml` was renumbered again in v0.3.7 (now v0.3.10/v0.3.11 after v0.3.8 and v0.3.9 also each got consumed by an inserted round) — keep these two in sync whenever a version renumbering happens again.
...
- `strings.xml`'s Tools/Explore placeholder copy is now v0.3.10/v0.3.11 (renumbered again since v0.3.9 also got consumed by an inserted round — see below) — keep these in sync on any future renumbering.
